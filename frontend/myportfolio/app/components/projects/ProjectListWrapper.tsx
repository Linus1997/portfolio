...
const createInitialState = ({ projects }: InitParam): State => {
  ...
  return {
    ...
    currentIndices: Array.from({ length: 6 }, (_, i) => i),
    newIndices: Array.from({ length: 6 }, (_, i) => i),
  ...
  switch (action.type) {
    ...
    case "moveLeft":
      ...
      return {
        ...state,
        itemData: moveLeft,
        currentIndices: rotateArray(state.currentIndices, -1),
        glowState: Variant.GLOW,
      };
    case "moveRight":
      let newIndices = rotateArray(state.newIndices, 1);
      let moveRight = rotateArray(state.itemData, 1).map((item, i) => {
        const itemData: ItemData = {
          ...item,
          pathTo: paths[newIndices[item.index]],
        };
        return itemData;
      });
